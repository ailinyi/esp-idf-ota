...
void bootloader_utility_load_boot_image(const bootloader_state_t *bs, int start_index)
{
    ...
    esp_partition_pos_t base_part;
    esp_image_metadata_t image_data;
    DO_SIZE_T datasss ;

    /////////////////////test////////////////////////////////////
    // esp_rom_spiflash_read(0x7C0000 , datasss,sizeof(datasss));

    // for (int i = 0; i < sizeof(datasss); i++) {
    //     ESP_LOGE(TAG, "datasss[%d]%d\n",i,datasss[i]);
    // }
    ...
    // esp_rom_spiflash_write(0x7C0000 ,datasss,sizeof(datasss));
    ...
    /////////////////////TRUE////////////////////////////////////
    esp_rom_spiflash_read(0x7C0000 , (uint32_t *)&datasss,sizeof(datasss));

    ESP_LOGE(TAG, "datasss.data0x%x\n",datasss.data);
    ESP_LOGE(TAG, "datasss.base_data0x%x\n",datasss.base_data);

    base_part.offset = datasss.base_data;
    base_part.size = 0x1E0000;
    part.offset = datasss.data;
    part.size =   0x1E0000;

    //假如基础是有问题的，那么初始化
    if(base_part.offset == 0xFFFFFFFF || base_part.offset == 0x00000000 ) {   
        datasss.base_data = base_part.offset = 0x400000; 
    } 
    //假如基础是有问题的，那么初始化
    if(part.offset == 0xFFFFFFFF ||part.offset == 0x00000000 ) {   
        datasss.data = part.offset = base_part.offset;
    } 

    //假如现在要设置的已经被修改过了，那么修改一下，下次重定向成我们的
    if(part.offset != datasss.base_data) {

        datasss.data = datasss.base_data;
    }

    esp_rom_spiflash_erase_sector(0x7C0000 /1024/4); //1,984  4KB对齐
    esp_rom_spiflash_write(0x7C0000 ,(uint32_t *)&datasss,sizeof(datasss));
    esp_rom_spiflash_read(0x7C0000 , (uint32_t *)&datasss,sizeof(datasss));
    ...
    ESP_LOGE(TAG, "******************************************");
    ESP_LOGE(TAG, "I can jump this");
    ESP_LOGE(TAG, "******************************************");
    // part.offset = 0x400000;
    // part.size   = 0x1E0000;
