...
esp_err_t esp_https_ota_begin(esp_https_ota_config_t *ota_config, esp_https_ota_handle_t *handle)
{
    ...
    https_ota_handle->update_partition = esp_ota_get_running_partition();
    ...
    update_partition.flash_chip = https_ota_handle->update_partition->flash_chip;
    update_partition.type = https_ota_handle->update_partition->type;
    update_partition.subtype = https_ota_handle->update_partition->subtype;
    update_partition.address = https_ota_handle->update_partition->address;
    update_partition.size = https_ota_handle->update_partition->size;
    memcpy(update_partition.label,https_ota_handle->update_partition->label,17);
    update_partition.encrypted = https_ota_handle->update_partition->encrypted;

    ESP_LOGI(TAG, "Writing to partition subtype %d at offset 0x%x",
        https_ota_handle->update_partition->subtype, https_ota_handle->update_partition->address);
    
    ESP_LOGI(TAG, "Writing to partition encrypted %d at label %s",
        https_ota_handle->update_partition->encrypted, https_ota_handle->update_partition->label);
    ...
    ESP_LOGI(TAG, "Writing to partition encrypted %d at label %s",
        update_partition.encrypted, update_partition.label);

    if(update_partition.address == 0x400000) {
        update_partition.address = 0x5e0000;
        update_partition.size = 0x1e0000;
        memset(update_partition.label , 0, 17);
        memcpy(update_partition.label,"factory_1",sizeof("factory_1"));
    } else  {
        update_partition.address = 0x400000;
        update_partition.size = 0x1e0000;
        memset(update_partition.label , 0, 17);
        memcpy(update_partition.label,"factory",sizeof("factory"));
    }
...
esp_err_t esp_https_ota_perform(esp_https_ota_handle_t https_ota_handle)
{
    ...
    switch (handle->state) {
        case ESP_HTTPS_OTA_BEGIN:
            err = esp_ota_begin(&update_partition, OTA_SIZE_UNKNOWN, &handle->update_handle);
            if (err != ESP_OK) {
...
esp_err_t esp_https_ota_finish(esp_https_ota_handle_t https_ota_handle)
{
    ...
    if (handle->state < ESP_HTTPS_OTA_BEGIN) {
        ...
        ESP_LOGE(TAG, "esp_https_ota_finish config not found");
    ...
    switch (handle->state) {
        ...
        ESP_LOGE(TAG, "esp_https_ota_finish config not found2");
    if ((err == ESP_OK) && (handle->state == ESP_HTTPS_OTA_SUCCESS)) {
        ...
        datasss.base_data = update_partition.address;
        
        ESP_LOGE(TAG, "datasss.data = %x",datasss.data);
        ESP_LOGE(TAG, "datasss.base_data = %x",datasss.base_data);
        esp_flash_t *chip = NULL;
        esp_flash_init(chip);
        esp_flash_erase_region(chip, 0x7C0000,  1024*4);
        esp_flash_write(chip, (uint8_t *)&datasss,  0x7C0000,  sizeof(datasss));
        esp_flash_read(chip, (uint8_t *)&datasss,  0x7C0000,  sizeof(datasss));
        ...
        //esp_rom_spiflash_write(0x7C0000 ,(uint8_t *)&datasss,sizeof(datasss));
            ESP_LOGE(TAG, "+++++++++++++++++++++++++++++++++");
        if (err != ESP_OK) {
...
esp_err_t esp_https_ota(const esp_http_client_config_t *config)
{
    ...
    ESP_LOGE(TAG, "esp_https_ota_finish step 1");
